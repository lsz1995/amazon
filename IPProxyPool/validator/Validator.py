...
def baidu_check(selfip, proxies):
    ...
    try:
        start = time.time()
        r = requests.get(url='https://www.amazon.cn/s?ie=UTF8&page=1&rh=n%3A664978051%2Ck%3A%E5%8D%8E%E4%B8%BA', headers=config.get_header(), timeout=config.TIMEOUT, proxies=proxies)
        r.encoding = chardet.detect(r.content)['encoding']
        if r.ok:
            speed = round(time.time() - start, 2)
            protocol= 0
